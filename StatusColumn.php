 ...
 */
class StatusColumn extends EditableColumn
{
    /**
    ...
    protected function initOptions()
    {
        ...
        foreach ($this->optionsArray as $option) {
            ...
            $this->labelClasses[$option[0]] = $option[2];
        }
        $userFunc = $this->value;
        ...
        $this->value = function ($model, $key, $index, $widget) use ($userFunc) {
            ...
            if (isset($widget->labelClasses[$attributeValue], $widget->filter[$attributeValue])) {
                $label = $widget->labelClasses[$attributeValue];
                ...
                if (!empty($this->toggleUrl)) {
                    ...
                    $idPlaceholder = ltrim($matches[0], '=');
                    $idAttribute = trim($idPlaceholder, '_');

                    $toggleUrl = str_replace($idPlaceholder, $model->{$idAttribute}, $this->toggleUrl);
