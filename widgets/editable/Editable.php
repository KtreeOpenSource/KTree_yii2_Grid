...
class Editable extends InputWidget
{
    ...
    protected $prepareToAutotext = false;
    /**
     * @see Editable
     * @var object
     */
    ...
    public function htmlOptions()
    {
        if (ArrayHelper::getValue($this->pluginOptions, 'toggle') == 'manual') {
            ...
            return Html::a(
                '<i class="glyphicon glyphicon-pencil" style="padding-right: 5px"></i>[edit]',
                '#',
                [
                    'id' => "edit-" . $this->id
                ]
    ...
    /**
     * @see    Editable
     * @see    Register input types
     *
     * @params editable Columns
     */
    public static function register($gridData)
    {
        if ($gridData->editableColumns) {
            Yii::$app->getView()->registerJs(
                '
            if($("#' . $gridData->options['id'] . '> .grid-table").find("table ").hasClass("table")){
             $("#' . $gridData->options['id'] . '>.grid-table > .table").Tabledit({
               url:"' . $gridData->gridEditAction . '",
               model:' . json_encode($gridData->model) . ',
               gridId:' . json_encode($gridData->options['id']) . ',
               columns: {
                   editable:' . json_encode($gridData->editableColumns) . '
               },
               deleteButton: false,
               saveButton:false
             });}'
